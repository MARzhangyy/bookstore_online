...
from bookstore_cart.models import CartInfo
from bookstore_user.models import BooksBrowser
...
def index(request):
    ...
    return render(request, 'bookstore_goods/index.html', context)
...
def good_list(request, tid, pindex, sort):
    ...
    return render(request, 'bookstore_goods/list.html', context)
...
def detail(request, gid):
    ...
    response = render(request, 'bookstore_goods/detail.html', context)

    if 'user_id' in request.session:
        user_id = request.session["user_id"]
        try:
            browsed_good = BooksBrowser.objects.get(user_id=int(user_id), good_id=int(good_id))
        except Exception:
            browsed_good = None
        if browsed_good:
            ...
            BooksBrowser.objects.create(user_id=int(user_id), good_id=int(good_id))
            browsed_goods = BooksBrowser.objects.filter(user_id=int(user_id))
...
def ordinary_search(request):
    ...
    if search_keywords:
        goods_list = GoodsInfo.objects.filter(
            ...
            Q(gdetails__icontains=search_keywords) |
            Q(gintroduction__icontains=search_keywords)).order_by("gclick")
    else:
        search_status = False
    ...
    return render(request, 'bookstore_goods/ordinary_search.html', context)
